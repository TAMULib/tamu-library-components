...
const NM_TL_UD_DIR = `${process.cwd()}/node_modules/@tl/elements/.tl-ud`;
const NM_UD_STATIC_ASSETS_DIR = `${NM_TL_UD_DIR}/static-assets`;
const TL_UD_DIR = `${process.cwd()}/.tl-ud`;
const TL_UD_STATIC_ASSETS_DIR = `${TL_UD_DIR}/static-assets`;
const CONFIG = require(`${TL_UD_DIR}/config.json`);
...
if(fs.existsSync(`${TL_UD_STATIC_ASSETS_DIR}/_styles.css`)) {
  stlyeFiles.push(`${TL_UD_STATIC_ASSETS_DIR}/_styles.css`);
};

if(fs.existsSync(`${TL_UD_STATIC_ASSETS_DIR}/overrides.css`)) {
  stlyeFiles.push(`${TL_UD_STATIC_ASSETS_DIR}/overrides.css`);
};

concat(stlyeFiles, `${TL_UD_STATIC_ASSETS_DIR}/styles.css`).finally(() => {
  const staticAssets = glob.sync(`${TL_UD_STATIC_ASSETS_DIR}/**/*`, {});
  ...
  if(!localWudExists) {
    const nmWud = `${NM_TL_UD_DIR}/static-assets/wud.js`;
  ...
  const localIndexBase = `${TL_UD_DIR}/static-assets/index-base.html`;
  const nmIndexBase = `${NM_TL_UD_DIR}/static-assets/index-base.html`;
